...
type
  WeirdListSetQueThing = object
    que:DoublyLinkedList[seq[int]]
    move_que:DoublyLinkedList[int]
  pair = tuple
    board:seq[int]
    move:int

var path:WeirdListSetQueThing
path.que.append(@[1,2,3])
...
proc GetLast(store:WeirdListSetQueThing):pair=
  return pair( store.move_que.head(), store.que.head())


#proc __init__(self,start):
#      self._que = [start]
#      self._move_que = [0]
#
#  def Show(self):
